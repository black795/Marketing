...
import { ExportPanel, RenderQueueProvider } from '@/render';
import Spinner from '@/components/loading/Spinner';

export default function ExportLoader({ projectId }: { projectId: string | null }) {
  const { status, project, error } = useStoryboardProject(projectId);

  if (!projectId) {
    return <EmptyState title="Sin proyecto" body="Abre el export pasando ?projectId=…" />;
  }
  if (status === 'loading' || status === 'idle') {
    return (
      <div className="flex h-[60vh] items-center justify-center gap-2 text-sm text-neutral-500">
        <Spinner size={18} className="text-brand-pink" /> Cargando proyecto…
      </div>
    );
  }
  if (status === 'error') return <EmptyState title="No se pudo cargar" body={error ?? 'Error'} tone="error" />;
  if (status === 'empty' || !project) {
    return (
      <EmptyState
        title="Proyecto sin escenas"
        body="Genera un guion + imágenes desde Scripts antes de exportar."
      />
    );
  }
