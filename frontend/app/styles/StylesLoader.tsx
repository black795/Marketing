...
export default function StylesLoader({ projectId }: { projectId: string | null }) {
  ...
  if (!projectId) {
    return (
      <EmptyState
        title="Sin proyecto"
        body="Abre el Style Engine pasando ?projectId=… en la URL. Lo normal es entrar desde el editor o el storyboard."
      />
    );
  }
  ...
  if (status === 'error') {
    return <EmptyState title="No se pudo cargar" body={error ?? 'Error desconocido'} tone="error" />;
  }
  if (status === 'empty' || !project) {
    return (
      <EmptyState
        title="Proyecto sin escenas"
        body="Genera un guion + imágenes desde Scripts antes de aplicar estilos."
      />
    );
  }
